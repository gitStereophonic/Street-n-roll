...
import { sendFeedback, dismissSendFeedbackError, reducer } from 'src/features/about-us/redux/sendFeedback';
...
describe('about-us/redux/sendFeedback', () => {
  ...
  //   return store.dispatch(sendFeedback()).then(() => {
  ...
  it('dispatches failure action when sendFeedback fails', () => {
    ...
    return store.dispatch(sendFeedback({ error: true })).catch(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property('type', ABOUT_US_SEND_FEEDBACK_BEGIN);
  ...
  it('handles action type ABOUT_US_SEND_FEEDBACK_BEGIN correctly', () => {
    ...
    const state = reducer(prevState, { type: ABOUT_US_SEND_FEEDBACK_BEGIN });
  ...
  it('handles action type ABOUT_US_SEND_FEEDBACK_SUCCESS correctly', () => {
    ...
    const state = reducer(prevState, { type: ABOUT_US_SEND_FEEDBACK_SUCCESS, data: {} });
    expect(state).to.not.equal(prevState); // should be immutable
  ...
  it('handles action type ABOUT_US_SEND_FEEDBACK_DISMISS_ERROR correctly', () => {
    ...
    const state = reducer(prevState, { type: ABOUT_US_SEND_FEEDBACK_DISMISS_ERROR });
    expect(state).to.not.equal(prevState); // should be immutable
