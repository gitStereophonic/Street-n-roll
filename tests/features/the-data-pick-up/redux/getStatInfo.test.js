...
import { getStatInfo, dismissGetStatInfoError, reducer } from 'src/features/the-data-pick-up/redux/getStatInfo';
...
describe('the-data-pick-up/redux/getStatInfo', () => {
  ...
  //   const args = { questionId: 1, what: { dbName: 'city', chartType: 'pie', extraField: null } };

  //   return store
  //     .dispatch(getStatInfo(args))
  //     .then(() => {
  ...
  it('dispatches failure action when getStatInfo fails', () => {
    ...
    return store.dispatch(getStatInfo({ questionId: 1, what: { dbName: 'city', chartType: 'pie', extraField: null }, error: true })).catch(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property('type', THE_DATA_PICK_UP_GET_STAT_INFO_BEGIN);
  ...
  it('handles action type THE_DATA_PICK_UP_GET_STAT_INFO_BEGIN correctly', () => {
    ...
    const state = reducer(prevState, { type: THE_DATA_PICK_UP_GET_STAT_INFO_BEGIN });
  ...
  //   const state = reducer(prevState, { type: THE_DATA_PICK_UP_GET_STAT_INFO_SUCCESS, data: {} });
  //   expect(state).to.not.equal(prevState); // should be immutable
  ...
  it('handles action type THE_DATA_PICK_UP_GET_STAT_INFO_DISMISS_ERROR correctly', () => {
    ...
    const state = reducer(prevState, { type: THE_DATA_PICK_UP_GET_STAT_INFO_DISMISS_ERROR });
    expect(state).to.not.equal(prevState); // should be immutable
